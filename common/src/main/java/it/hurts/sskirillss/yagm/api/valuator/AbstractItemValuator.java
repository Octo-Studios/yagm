...
public abstract class AbstractItemValuator {
    ...
    protected void computeRecipeValues(List<Recipe<?>> recipes) {
        ...
        do {
            ...
            if (iterations > maxIterations) {
                log.warn("Recipe value computation reached max iterations ({}). Possible circular dependency detected.", maxIterations);
                break;
            }
    ...
    private void scheduleRecipeComputation() {
        ...
        Thread worker = new Thread(() -> {
            ...
            fillMissingValues();
            exportValues();
            log.info("ItemValuator recipe values computed in {} ms ({} recipes).", System.currentTimeMillis() - start, recipesSnapshot.size());}, "yagm-item-valuator");
