...
public class NeoForgeBackPackedCompat extends BaseBackpackCompat {
    ...
    public List<BackpackEntry> collectBackpacks(ServerPlayer player) {
        ...
        boolean keepOnDeath = readConfig();
    ...
    protected boolean restoreLeft(ServerPlayer player, BackpackEntry backpack) {
        int parseindex = parseIndex(backpack.slotKey());

        if (parseindex < 0 || !BackpackHelper.getBackpackStack(player, parseindex).isEmpty()) {
            return false;
        }

        return BackpackHelper.setBackpackStack(player, backpack.stack().copy(), parseindex);
    }
